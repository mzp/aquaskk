...
skkserv_impl::skkserv_impl(unsigned short port, bool localonly)
    : thread_(0), localonly_(localonly) {
    server_.open(port);

    if(server_) {
        incoming_.add(server_.socket(), net::socket::monitor::READ);
        pthread_create(&thread_, 0, skkserv_impl::listener, this);
    } else {
        std::cerr << "skkserv::start(): can't open port " << port << std::endl;
    }
}

skkserv_impl::~skkserv_impl() {
...
void *skkserv_impl::listener(void *param) {
    skkserv_impl *server = reinterpret_cast<skkserv_impl *>(param);
...
void skkserv_impl::accept() {
    if(0 < incoming_.wait()) {
        ...
        pthread_create(&tmp, 0, skkserv_impl::worker, param);
    }
}

void *skkserv_impl::worker(void *arg) {
