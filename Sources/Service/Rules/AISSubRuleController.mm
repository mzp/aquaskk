...
- (instancetype)initWithPath:(NSString *)path
                 activeRules:(NSArray *)activeRules {
    self = [super init];
    if (self) {
        self.path = path;
...
- (NSArray<AISSubRule *> *)initializeSubRulesAtPath {
    SubRuleDescriptions *table =
        new SubRuleDescriptions([self.path UTF8String]);
    NSDirectoryEnumerator *files =
        [[NSFileManager defaultManager] enumeratorAtPath:self.path];
    ...
    while (NSString *file = [files nextObject]) {
        if ([[file pathExtension] isEqualToString:@"rule"]) {
            ...
            rule.ruleDescription = [NSString
                stringWithUTF8String:table->Description([file UTF8String])
                                         .c_str()];
            ...
            BOOL enabled =
                self.activeRules != nil
                    ? [self.activeRules
                          containsObject:
                              [self.path stringByAppendingPathComponent:file]]
                    : NO;
    ...
    [rules sortUsingComparator:^NSComparisonResult(AISSubRule *rule1,
                                                   AISSubRule *rule2) {
