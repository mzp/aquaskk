...
- (AISJisyoType)type {
    return (AISJisyoType)((NSNumber *)self.dictionary[SKKDictionarySetKeys::type]).integerValue;
}

- (BOOL)enabled {
    return ((NSNumber *)self.dictionary[SKKDictionarySetKeys::active]).boolValue;
}
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<AISJisyo: type=%@;location=%@;enabled=%@>", self.displayType, self.location,
                                      @(self.enabled)];
}
