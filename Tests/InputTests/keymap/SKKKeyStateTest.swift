...
struct SKKKeyStateTest {
    ...
    @Test func keycode() {
        let keyState = SKKKeyState.KeyCode(0x31, SKKKeyModifier.shift.rawValue)
        #expect(keyState.charCode == 0)
        #expect(keyState.keyCode == 0x31)
        #expect(keyState.mods == SKKKeyModifier.shift.rawValue)
    }

    @Test func charCode() {
        let keyState = SKKKeyState.CharCode(0x31, SKKKeyModifier.shift.rawValue)
    ...
    @Test func equals() {
        let keyState1 = SKKKeyState.KeyCode(0x31, SKKKeyModifier.shift.rawValue)
        let keyState2 = SKKKeyState.KeyCode(0x31, SKKKeyModifier.shift.rawValue)
